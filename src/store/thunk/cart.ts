...
const deleteItemFromCartThunk = createAsyncThunk<
  CartResponse,
  DeleteFromCartPayload,
  { rejectValue: string }
>("cart/items/delete", async (payload, { rejectWithValue }) => {
  try {
...
const getCartByIdThunk = createAsyncThunk<
  CartResponse,
  string,
  { rejectValue: string }
>("cart/cartById", async (cartId, { rejectWithValue }) => {
  try {
    return await cartById(cartId);
  } catch (error) {
    const axiosError = error as AxiosError<any>;
    return rejectWithValue(
      axiosError?.response?.data?.message ?? "Failed to fetch cart details"
    );
  }
});

const updateShippingAddressForCartThunk = createAsyncThunk<
  CartResponse,
  ShippingAddress & { cartId: string },
  { rejectValue: string }
>("cart/updateShippingAddress", async (payload, { rejectWithValue }) => {
  try {
    const { cartId, ...shippingDetails } = payload;
    return await updateShippingAddressForCart(cartId, shippingDetails);
  } catch (error) {
    const axiosError = error as AxiosError<any>;
    return rejectWithValue(
      axiosError?.response?.data?.message ?? "Failed to update shipping address"
    );
  }
});
